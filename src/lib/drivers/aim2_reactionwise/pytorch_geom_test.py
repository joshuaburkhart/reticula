...
class Net(torch.nn.Module):
    def __init__(self):
        ...
        self.conv1 = GCNConv(2, 16)  # dataset.num_node_features, 16)  # 2 features per node, 13 examples of the graph
        self.conv2 = GCNConv(16, 4)  # dataset.num_classes)  # 13 examples of the graph, 4 classes of graph
...
for epoch in range(200):
    optimizer.zero_grad()
    out = model(data)
    loss = F.nll_loss(out, torch.tensor(numpy.repeat(data.y,3)))
    loss.backward()
    optimizer.step()
