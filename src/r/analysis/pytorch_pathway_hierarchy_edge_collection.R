...
tissue2idx.df <- data.frame(read.table(paste(IN_DIR,"pathway_hierarchy_inverted_targets.txt",sep=""),
                            ...
                            read.table(paste(IN_DIR,"pathway_hierarchy_transformed_targets.txt",sep=""),
                                       stringsAsFactors = FALSE)) %>%
  unique()
...
node2idx.df <- data.frame(read.table(paste(IN_DIR,"pathway_hierarchy_nodeLabel2rxn_nls.csv",sep="")),
                          read.table(paste(IN_DIR,"pathway_hierarchy_rxn2nodeLabel_nls.csv",sep="")))
...
edge2idx.df <- data.frame(read.table(paste(IN_DIR,"pathway_hierarchy_edges.txt",sep="")),
                          read.table(paste(IN_DIR,"pathway_hierarchy_edgeLabels.csv",sep="")))
colnames(edge2idx.df) <- c("Preceeding_Index","Following_Index","Preceeding_Reaction","Following_Reaction")

ig_edge_idx.df <- data.frame(read.table(paste(PWAY_EDGE_DIR, "ig_0.txt",sep="")))
ig_colnames <- c(paste("IG_",tissue_name(0),sep=""))
for(i in 1:50) {
  ig_edge_idx.df <- cbind(ig_edge_idx.df,read.table(paste(PWAY_EDGE_DIR,"ig_",i,".txt",sep="")))
  ig_colnames <- c(ig_colnames,paste("IG_",tissue_name(i),sep=""))
}
colnames(ig_edge_idx.df) <- ig_colnames

saliency_edge_idx.df <- data.frame(read.table(paste(PWAY_EDGE_DIR, "saliency_0.txt",sep="")))
saliency_colnames <- c(paste("Saliency_",tissue_name(0),sep=""))
for(i in 1:50) {
  saliency_edge_idx.df <- cbind(saliency_edge_idx.df,read.table(paste(PWAY_EDGE_DIR,"saliency_",i,".txt",sep="")))
...
write.csv(labelled_edge_weights.df,file = paste(IN_DIR,"pathway_hierarchy_labelled_edge_weights.csv"))