...
N_FOLDS <- 10
tcga_zsl_counts_fn <- "tcga_vst_counts.Rds"
...
#DESeq2 PCA plot
vst.counts <- readRDS(paste(TCGA_OUT_DIR, tcga_zsl_counts_fn, sep = ""))

vst.count.mtx <-
   vst.counts %>% SummarizedExperiment::assay() %>% as.data.frame()
tcga_tissue.vec <-
   readRDS(paste(TCGA_OUT_DIR, "tcga_tissue_vec.Rds", sep = ""))
rxn2ensembls.nls <-
   readRDS(paste(TCGA_OUT_DIR, "rxn2ensembls_nls.Rds", sep = ""))
...
saveRDS(tcga_tissue.vec.train,file=paste(TCGA_OUT_DIR,"zsl_tcga_tissue_vec_train.Rds",sep=""))
...
for(rxn_id_idx in seq(1:n_rxns)){
   ...
   if(mod(rxn_id_idx,1000) == 0){
      ...
      saveRDS(full_rxn_pca_results.nls,
              paste(TCGA_OUT_DIR, "zsl_full_rxn_pca_results_nls",rxn_id_idx-1000,
                    "-",rxn_id_idx,".Rds", sep=""))
      full_rxn_pca_results.nls <- list()
      gc()
...
saveRDS(rxn_id_2_result_file_idx.nls,
        paste(TCGA_OUT_DIR,"zsl_rxn_id_2_result_file_idx_nls.Rds",sep=""))
saveRDS(full_rxn_pca_results.nls,
        paste(TCGA_OUT_DIR, "zsl_full_rxn_pca_results_nls.Rds", sep=""))
...
saveRDS(
   ...
   paste(TCGA_OUT_DIR, "zsl_toi_rxn_knn_misclass_rate_nls.Rds", sep = "")
)
saveRDS(rxn_knn_ari.nls,
        paste(TCGA_OUT_DIR, "zsl_toi_rxn_knn_ari_nls.Rds", sep = ""))
saveRDS(rxn_knn_ecount.nls,
        paste(TCGA_OUT_DIR, "zsl_toi_rxn_knn_ecount_nls.Rds", sep = ""))
...
saveRDS(d, paste(TCGA_OUT_DIR, "zsl_toi_summary_df.Rds", sep = ""))
