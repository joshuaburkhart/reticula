...
REACTION_TO_PTHWY_FN <- "/home/jgburk/PycharmProjects/reticula/data/SRP049593/input/ReactionToPathway_Rel_71_122820.txt" # on box.com
...
TOP_N = 20
top_weighted_rxns_tall <- labelled_edge_weights.df %>%
  dplyr::slice_max(labelled_edge_weights.df$`IG_tissue: T cell acute lymphoblastic leukemia`,n=TOP_N) %>%
  dplyr::select(Preceeding_Reaction)

top_weighted_rxn_tall_ensembls <- sort(unique(unlist(rxn2ensembls.nls[unlist(top_weighted_rxns_tall)])))
write.table(top_weighted_rxn_tall_ensembls,
            file=paste(IN_DIR,"top_weighted_rxn_tall_ensembls.txt",sep=""),
            row.names = FALSE,
            col.names = FALSE,
            quote = FALSE,
            sep="\n")

top_weighted_rxns_thymus <- labelled_edge_weights.df %>%
  dplyr::slice_max(labelled_edge_weights.df$`IG_tissue: normal thymus`,n=TOP_N) %>%
  dplyr::select(Preceeding_Reaction)

top_weighted_rxn_thymus_ensembls <- sort(unique(unlist(rxn2ensembls.nls[unlist(top_weighted_rxns_thymus)])))
write.table(top_weighted_rxn_thymus_ensembls,
            file=paste(IN_DIR,"top_weighted_rxn_thymus_ensembls.txt",sep=""),
            row.names = FALSE,
            col.names = FALSE,
            quote = FALSE,
            sep="\n")
