...
srp049593_tissue.vec <-
   readRDS(paste(OUT_DIR, "srp049593_tissue_vec.Rds", sep = ""))
rxn2ensembls.nls <-
...
toi_indices <- seq(1,length(srp049593_tissue.vec))

# filter annotations
srp049593_tissue_vec_tis_of_interest <-
   srp049593_tissue.vec[toi_indices]
...
srp049593_tissue.vec.train <-
   srp049593_tissue_vec_tis_of_interest[training_indices]
srp049593_tissue.vec.test <-
   srp049593_tissue_vec_tis_of_interest[-training_indices]

saveRDS(srp049593_tissue.vec.train,file=paste(OUT_DIR,"srp049593_tissue_vec_train.Rds",sep=""))

cv_fold_indices <- caret::createFolds(srp049593_tissue.vec.train,
                                      k = N_FOLDS)
binary_srp049593_tissue_annotations <- unique(srp049593_tissue.vec)
...
for (rxn_id_idx in seq(1:length(rxns))) {
   ...
   for (cv_fold in names(cv_fold_indices)) {
      ...
      srp049593_tissue.vec.train.cv_train <-
         srp049593_tissue.vec.train[-cur_cv_fold_indices] # 4/5ths of training labels
      srp049593_tissue.vec.train.cv_test <-
         srp049593_tissue.vec.train[cur_cv_fold_indices] # 1/5th of training labels
      
      binary_srp049593_tissue_vec.test.cv_test_list <- list()
      for (tissue_annotation in binary_srp049593_tissue_annotations) {
         binary_srp049593_tissue_vec.test.cv_test_list[[tissue_annotation]] <-
            (srp049593_tissue.vec.train.cv_test == tissue_annotation)
      }
      ...
      rxn_knn_calls <- class::knn(train = cv_train.expr_mat,
                                  ...
                                  cl = srp049593_tissue.vec.train.cv_train)
   ...
   if (mod(count, 10) == 0) {
      print(
         paste(
            ...
            ecount,
            ": Last male MISCLASS = ",
            rxn_knn_misclass_rate.nls[[rxn_id]][["male"]],
            ": Last female MISCLASS = ",
            rxn_knn_misclass_rate.nls[[rxn_id]][["female"]],
            ". Now ",
...
pca.d <- data.frame(
   ...
   Section = srp049593_tissue.vec.train
)
ggplot(pca.d) +
